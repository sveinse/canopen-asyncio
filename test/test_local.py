...
class BaseTests:
    ...
    class TestSDO(unittest.IsolatedAsyncioTestCase):
        ...
        async def test_receive_abort_request(self):
            if self.use_async:
                # FIXME: No native support for abort in async mode
                await asyncio.to_thread(self.remote_node.sdo.abort, 0x05040003)
            else:
...
class TestPDOSync(BaseTests.TestPDO):
    use_async = False


class TestPDOAsync(BaseTests.TestPDO):
    use_async = True
